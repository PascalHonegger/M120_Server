		...
		public IEnumerable<ChatMessage> Messages
		{
			get
			{
				...
				using (var connection = new SqlConnection(Settings.Default.ConnectionString))
				{
					...
					var command =
						new SqlCommand(
							"SELECT id_message FROM [message] INNER JOIN [message_user] ON id_message = fk_message WHERE fk_chatroom IS NULL AND ((fk_user = @user AND author = @user2) OR (fk_user = @user2 AND author = @user))",
							connection);
